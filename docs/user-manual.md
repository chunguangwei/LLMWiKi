...
### 5.6 docx / Office 源文件预览（0.4.15）

**入口：左侧「资料」面板点选文件**

以前点开 `raw/sources/` 里的 Word / Office 原始文件会提示「该文件类型暂不支持预览」。现在直接在右侧预览面板显示**提取出的文本**,方便你在摄入前后核对原文。

- 支持：`.docx` `.xlsx` `.pptx` `.odt` `.ods` `.odp`
- 显示的是纯文本提取结果(用于快速查看/比对),不是原版排版;复杂表格/图片以摘要文本呈现
- 无需任何配置,点选即看

### 5.7 页面类型选择器 —— 即时调整左侧分组（0.4.17）

**入口：打开任一 wiki 页面 → 预览面板顶部的「类型」徽章**

左侧知识树是按每个页面的 `type`(类型)分组的(书籍 / 概念 / 旅游方案 / 论文…)。如果某页被分到了不合适的组,**不必再去手改 frontmatter**:

1. 在左侧点开这个 wiki 页面
2. 预览面板顶部的类型徽章现在是一个**下拉框**(读模式、编辑模式都能点)
3. 选一个目标分类(显示的是中文标签)
4. 程序立即把对应英文 `type` slug 写回该页 frontmatter,**实时把页面挪到左侧对应分组**,并**同步更新 `wiki/index.md` 目录**——把该页条目移到新分类章节(0.4.18 起)

要点：

- 选项就是综合 schema 的全部类型 + 旧类型 + 研究类型,和左侧树分组一一对应
- 写回是即时单次操作,不影响你正文的自动保存
- 如果某页原本是自定义/旧 slug(下拉里没有的),会作为额外项保留显示,不会被悄悄清掉
- 只对 `wiki/` 下的页面生效(`index.md` / `log.md` 和项目根的 `purpose.md` / `schema.md` 不显示该下拉)
- `overview`(全局概览页)是自动管理的,不在可选项里
- **边界:只改"分类"(`type` + 左侧分组 + index.md 章节),不移动磁盘文件**。所以改完后,文件仍在原目录(如 `wiki/书籍/`),而类型/分组可能已是"概念"——磁盘目录与逻辑分类会出现不一致。这不影响功能(知识树、图谱按 `type` 取,页面链接按文件名 slug,均不依赖目录);只是若你也想让物理目录跟着变,需要手动移动文件并修正引用,或改 `schema.md` 后重新摄入。

> 这是「调某页归哪个分组」最省事的方式;批量重新分类、或想让物理目录也归位,仍建议改 `schema.md` 后重新导入(见 [`user-rules.md`](user-rules.md))。

### 5.8 在对话里直接修补 wiki（0.4.21）

**入口：右侧对话面板 → 让助手纠正/更新某页内容**

以前在对话里问 LLM "这段写错了,应该是 X",它给了建议你还得**手动复制回 `.md` 文件**。现在符合 Karpathy「LLM 维护、人只审核」理念的做法是:

1. 打开你想修的 wiki 页面(它会作为上下文自动传给对话,对话顶部能力知道"当前这页"),或直接在对话里描述要改哪部分知识。
2. 明确让助手**纠正 / 更新 / 重写 / 修补**(例如:"把这页里关于 X 的说法改成 Y""这页缺了 Z,补上")。
3. 助手回答后,下方会出现一张**改动卡片**:
   - 显示目标页路径 + `+新增 / −删除` 行数;
   - 点「Show diff」看**旧/新逐行对比**;
   - 点「Apply edit」**一键写回**。
4. 应用时:**先自动备份旧版本**到 `.llm-wiki/page-history/`(可恢复),保留页面的 `created` 日期,写回后左侧树/预览实时刷新,并记一条 `log.md`。

**不确定时走人工审核**:如果助手**判断不出该改哪一页**(描述含糊、跨多页、或没有匹配的现有页),它不会乱改,而是给出一张**蓝色「建议(需审核)」卡片**,点「Send to Review」把它送进左侧的「审核」面板,由你决定。

要点:

- **什么都不点就不会动你的 wiki**——卡片只是提议,应用与否你说了算(这本身就是人在环的审核)。
...
| 某页分到了错误的分组 | 打开它 → 预览面板顶部「类型」下拉里选对的分类，即时归位（见 §5.7） |
| 原始 Word/Office 文件点开看不到内容 | 0.4.15 起已支持，升级到最新版即可预览提取文本（见 §5.6） |
...
| [`app/README_CN.md`](../README_CN.md) | upstream 完整功能列表（含我们追加的 §19/20/21） |
