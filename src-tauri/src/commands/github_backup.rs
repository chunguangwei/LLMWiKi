...
// ---------------------------------------------------------------------------
// OAuth device flow — lets the user authorize without pasting a PAT. The
// frontend supplies a GitHub OAuth App client_id; we run the device-code
// handshake and stash the resulting token in the keychain.
// ---------------------------------------------------------------------------
...
/// POST https://github.com/login/device/code — begin the device flow. The UI
/// shows `user_code` + `verification_uri` to the user and polls below.
#[tauri::command]
pub async fn github_oauth_device_start(client_id: String, scope: String) -> Result<DeviceStart, String> {
    ...
    let resp = http_client()?
        ...
        .send()
        .await
        .map_err(|e| format!("device_start failed: {e}"))?;
    if !resp.status().is_success() {
        return Err(err_body(resp).await);
    }
    let v: serde_json::Value = resp.json().await.map_err(|e| format!("device_start parse: {e}"))?;
    Ok(DeviceStart {
        device_code: v.get("device_code").and_then(|x| x.as_str()).unwrap_or_default().to_string(),
        user_code: v.get("user_code").and_then(|x| x.as_str()).unwrap_or_default().to_string(),
        verification_uri: v.get("verification_uri").and_then(|x| x.as_str()).unwrap_or_default().to_string(),
...
/// POST https://github.com/login/oauth/access_token — poll once. The frontend
/// calls this every `interval` seconds until status is authorized/error. On
/// authorization we save the token to the keychain so the rest of the
/// commands can read it; we still echo it back so the UI can confirm.
#[tauri::command]
pub async fn github_oauth_device_poll(client_id: String, device_code: String) -> Result<DevicePoll, String> {
    ...
    let v: serde_json::Value = resp.json().await.map_err(|e| format!("device_poll parse: {e}"))?;

    if let Some(token) = v.get("access_token").and_then(|x| x.as_str()) {
        save_github_token(token.to_string())?;
        return Ok(DevicePoll { status: "authorized".to_string(), token: Some(token.to_string()) });
    }
    // GitHub signals progress/back-off via the `error` field.
    ...
    Ok(DevicePoll { status: status.to_string(), token: None })
}
